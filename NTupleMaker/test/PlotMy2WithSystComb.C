        ...
        )
{
    ...
    if (channel=="Wmnu")	file = new TFile(directory+"RootWithWeights"+"/"+Run+"/"+Variable+".root");
    file_1 = new TFile(directory+"RootWithWeights"+"/ABCD/"+Variable+".root");

    TH1D * histData = (TH1D*)file_1->Get("data_obs_"+Variable+suffix);	   
 
    TH1D * DY = (TH1D*)file_1->Get("DY_"+Variable+suffix); 
    TH1D * QCD = (TH1D*)file_1->Get("QCD_"+Variable+suffix); 
    TH1D * VV = (TH1D*)file_1->Get("VV_"+Variable+suffix); 
    TH1D * ST = (TH1D*)file_1->Get("ST_"+Variable+suffix); 
    TH1D * TT = (TH1D*)file_1->Get("TT_"+Variable+suffix); 
    TH1D * WJ = (TH1D*)file_1->Get("WJ_"+Variable+suffix); 

/*
    TH1D * histData0 = (TH1D*)file_1->Get("data_obs0_"+Variable+suffix);
    TH1D * DY0 = (TH1D*)file_1->Get("DY0_"+Variable+suffix); 
    TH1D * QCD0 = (TH1D*)file_1->Get("QCD0_"+Variable+suffix); 
    TH1D * VV0 = (TH1D*)file_1->Get("VV0_"+Variable+suffix); 
    TH1D * ST0 = (TH1D*)file_1->Get("ST0_"+Variable+suffix); 
    TH1D * TT0 = (TH1D*)file_1->Get("TT0_"+Variable+suffix); 
    TH1D * WJ0 = (TH1D*)file_1->Get("WJ0_"+Variable+suffix);

    histData->Add(histData0);
    DY->Add(DY0);
    QCD->Add(QCD0);
    VV->Add(VV0);
    ST->Add(ST0);
...
//	if(!logY && Variable.Contains("pt")) h = CreateAxisHists(2, histData, 0, 100-0.01);
    ...
    pads[1]->cd();
    h[1]->Draw();    
    SetupTwoPadSplitAsRatio(pads, "Data/MC", true, 0, 2.2);
    StandardAxes(h[1]->GetXaxis(), h[0]->GetYaxis(),xtitle_ ,units);
    h[1]->GetYaxis()->SetNdivisions(4);
    h[1]->GetXaxis()->SetTitleOffset(1.3);
    h[1]->GetYaxis()->SetTitleOffset(2.1);
    pads[0]->cd();
...
if (syst)
{
    ...
    //WJ:
    cout<<"WJ_JetResUp: "<<WJ_JetResUp->GetSumOfWeights()<<endl;
    cout<<"WJ_JetResDown: "<<WJ_JetResDown->GetSumOfWeights()<<endl;
    cout<<"WJ_JetEnUp: "<<WJ_JetEnUp->GetSumOfWeights()<<endl;
    cout<<"WJ_JetEnDown: "<<WJ_JetEnDown->GetSumOfWeights()<<endl;
    cout<<"WJ_UnclusteredEnUp: "<<WJ_UnclusteredEnUp->GetSumOfWeights()<<endl;
    cout<<"WJ_UnclusteredEnDown: "<<WJ_UnclusteredEnDown->GetSumOfWeights()<<endl;
...
if (syst)
{
     ...
     cout<<"eSyst_JetRes : "<<eSyst_JetRes<<endl;
	 cout<<"eSyst_JetEn  : "<<eSyst_JetEn<<endl;
	 cout<<"eSyst_UnclusteredEn : "<<eSyst_UnclusteredEn<<endl;
    ...
    float Err = TMath::Sqrt(eStat*eStat+
                            ...
                            eSyst_UnclusteredEn*eSyst_UnclusteredEn+
                            eMuon*eMuon);

	float Err1 = TMath::Sqrt(eStat*eStat+
                             eMuon*eMuon+
                             eLumi*eLumi);

	float Err2 = TMath::Sqrt(eStat*eStat+
                             eLumi*eLumi+
                             eMuon*eMuon+
                             eSyst_JetRes*eSyst_JetRes);

    float Err3 = TMath::Sqrt(eStat*eStat+
                             eLumi*eLumi+
                             eMuon*eMuon+
                             eSyst_JetRes*eSyst_JetRes+
                             eSyst_JetEn*eSyst_JetEn);
        ...
        //cout << eStat<< "  "<<eLumi <<"  "<< eMuon<<"  " << eSyst_JetRes<<"  " << 
        //eSyst_JetEn<<"  " << eSyst_UnclusteredEn<<"  " << iB<<endl;
        //cout << Err<< " |  "<<Err1 <<" | " << Err2<<" | " << Err3<<" | " <<endl;

        bkgdErr1->SetBinError(iB,Err1);
        bkgdErr2->SetBinError(iB,Err2);
        bkgdErr3->SetBinError(iB,Err3);
        bkgdErr4->SetBinError(iB,Err);

    }

    
    bkgdErr->SetMarkerSize(0);
    bkgdErr1->SetMarkerSize(0);
    bkgdErr2->SetMarkerSize(0);
    bkgdErr3->SetMarkerSize(0);
    bkgdErr4->SetMarkerSize(0);  
    ...
    int new_idx4 = CreateTransparentColor(kYellow+1,1);
    ...
    bkgdErr4->SetFillColor(new_idx4);    

    //bkgdErr->SetFillStyle(3002);
    bkgdErr1->SetFillStyle(3002);
    bkgdErr2->SetFillStyle(3002);
    bkgdErr3->SetFillStyle(3002);
    bkgdErr4->SetFillStyle(3002);

    //bkgdErr->SetLineWidth(1);
    bkgdErr1->SetLineWidth(1);
    bkgdErr2->SetLineWidth(1);
    bkgdErr3->SetLineWidth(1); 
    bkgdErr4->SetLineWidth(1);
   //bkgdErr->Draw("e2same");     
   //bkgdErr4->Draw("e2same");    
   //bkgdErr3->Draw("e2same");
   //bkgdErr2->Draw("e2same");
   //bkgdErr1->Draw("e2same");
    ...
    legend->AddEntry(bkgdErr4, "+ Uncl " , "F" );
    }
    ...
    TH1D * ratioErrH4 = (TH1D*)bkgdErr4->Clone("ratioErrH4");
    ...
    for (int iB=1; iB<=nBins; ++iB) {
        ...
        ratioErrH1->SetBinContent(iB,1.0);
    	ratioErrH2->SetBinContent(iB,1.0);
    	ratioErrH3->SetBinContent(iB,1.0);    
        ratioErrH4->SetBinContent(iB,1.0);

        ratioErrH->SetBinError(iB, 0.0);
        ratioErrH1->SetBinError(iB,0.0);
        ratioErrH2->SetBinError(iB,0.0);
        ratioErrH3->SetBinError(iB,0.0);      
        ratioErrH4->SetBinContent(iB,0.0);    
        ...
        float errBkg4 = bkgdErr4->GetBinError(iB);
    ...
    pads[1]->cd();
    ratioErrH->GetYaxis()->SetRangeUser(0.0,2.1);
    ...
    if (Variable.Contains("MT")) line = new TLine( 0,1, 200-0.01,1);
    ...
    if (channel=="Wmnu") canv1->Print("/nfs/dust/cms/user/dydukhle/STAU/METStudy/CMSSW_10_2_15_patch2/src/DesyTauAnalyses/NTupleMaker/test/WmnuPlots/RootWithWeights/Final/ABCD/Final_"+Variable+".pdf"    );
